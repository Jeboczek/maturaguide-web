...
class QuizPicker {
        ...
        switch (thisObj.mode) {
            ...
            if (choiceId == 0) {
                ...
                if (choiceId == 0) {
                    ...
                    window.location.href = `/play?subject=${thisObj.pickedSubject}&${thisObj.pickedMode == 1 ? "category=" + thisObj.pickedCategory : "cke_year=" + thisObj.pickedCategory}`
                }
                }
            });
        }
  ...
  async showTraningCards(subject_id){
      ...
      return $.getJSON(`/api/get_categories?subject_id=${subject_id}`, "",
  ...
  async showSheetCards(subject_id){
      const getTypeCardHTMLFromData = this.getTypeCardHTMLFromData
      return $.getJSON(`/api/get_cke_sheets?subject_id=${subject_id}`, "",
          function (data, textStatus, jqXHR) {
              let htmlToAdd = getTypeCardHTMLFromData({id: 0, content: "wróć"});;
                data.forEach((element) => {
                    // Reformat
